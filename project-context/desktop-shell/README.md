...
- Packaged builds keep all writable data under the Electron userData folder (Windows %APPDATA%\August, macOS ~/Library/Application Support/August) via AUGUST_DATA_DIR, WHISPER_MODEL_PATH and WHISPER_BIN_PATH env vars set before the server module is imported.
- whisper-cli is bundled from whisper/bin/<platform>/ into resources/whisper/bin/ as extraResources; the ~874 MB model is never bundled and auto-downloads on first launch.
...
- process.env.NODE_ENV, AUGUST_DATA_DIR, WHISPER_MODEL_PATH and WHISPER_BIN_PATH must be set before the server module is dynamically imported, because server/src/config.ts reads them at module load.
...
- macOS GUI apps do not inherit the shell PATH, so the shell appends /opt/homebrew/bin and /usr/local/bin before starting the server (ffmpeg discovery).
